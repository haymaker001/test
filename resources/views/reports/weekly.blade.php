@extends('layouts.app')
    ...
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        ...
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <!--begin::Title-->
            <h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Informe de Viajes Semanales</h1>
            <!--end::Title-->
            <!--begin::Separator-->
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                ...
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item text-dark">Informe de Viajes Semanales</li>
                <!--end::Item-->
            </ul>
            <!--end::Breadcrumb-->
        </div>
...
<div class="post d-flex flex-column-fluid" id="kt_post">
    ...
    <div id="kt_content_container" class="container-xxl">
        ...
        <form action="{{ route('reports.weekly') }}" method="POST">
            @csrf
            <!--begin::Card-->
            <div class="card mb-7">
                <!--begin::Card body-->
                        ...
                        <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example_1">
                            ...
                            <tbody class="fw-bold text-gray-600">
                                ...
                                <tr class="odd">
                                    ...
                                    <td>{{ $location->dolly }}</td>
                                    <td>{{ $location->normal }}</td>
                                    <td>{{ number_format($location->normal + $location->dolly) }}</td>
                                </tr>
                                @endforeach
                                @if($locations->count() > 0)
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase">
                                    <th><strong>TOTAL GENERAL: </strong></th>
                                    <th><strong>{{ number_format($locations->sum('dolly')) }}</strong></th>
                                    <th><strong>{{ number_format($locations->sum('normal')) }}</strong></th>
                                    <th><strong>{{ number_format($locations->sum('dolly') + $locations->sum('normal')) }}</strong></th>
                                </tr>
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase">
                                    <th><strong>% PARTICIPACION: </strong></th>
                                    <th><strong>{{ number_format( ($locations->sum('dolly') / ($locations->sum('dolly') + $locations->sum('normal')) ) * 100, 2) }} %</strong></th>
                                    <th><strong>{{ number_format( ($locations->sum('normal') / ($locations->sum('dolly') + $locations->sum('normal')) ) * 100, 2) }} %</strong></th>
                                    <th></th>
                                </tr>
                                @endif
                            </tbody>
    ...
    <script>
        ...
        var KTDatatablesButtons = function () {
            ...
            // Hook export buttons
            var exportButtons = () => {
                const documentTitle = 'Informe de Viajes Semanales - Periodo Desde: ' + from + ' Hasta: ' + to;
