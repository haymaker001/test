@extends('layouts.app')
    ...
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        ...
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <!--begin::Title-->
            <h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Informe de Combustible</h1>
            <!--end::Title-->
            <!--begin::Separator-->
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                <!--begin::Item-->
                <li class="breadcrumb-item text-muted">Combustible</li>
                <!--end::Item-->
                <!--begin::Item-->
                ...
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item text-dark">Informe de Combustible</li>
                <!--end::Item-->
            </ul>
            <!--end::Breadcrumb-->
        </div>
...
<div class="post d-flex flex-column-fluid" id="kt_post">
    ...
    <div id="kt_content_container" class="container-xxl">
        ...
        <form action="{{ route('reports.consumptions') }}" method="POST">
            @csrf
            <!--begin::Card-->
            <div class="card mb-7">
                <!--begin::Card body-->
                <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap gap-5 mb-2">
                        ...
                        <div class="fv-row w-100 flex-md-root">
                            <!--begin::Label-->
                            <label class="required form-label" for="supply_center_id">Centro de Abastecimiento</label>
                            <!--end::Label-->
                            <!--begin::Select2-->
                            <select class="form-select form-select-solid" name="supply_center_id" id="supply_center_id" data-control="select2" data-placeholder="seleccionar item">
                                <option></option>
                                @foreach($supply_centers as $center)
                                <option value="{{ $center->id }}" >{{ $center->name }}</option>
                                @endforeach
                            </select>
                            <!--end::Select2-->
                            <!--begin::Description-->
                            <div class="text-muted fs-7">Nombre del Centro de Abastecimiento.</div>
                            <!--end::Description-->
                        </div>                        
                    </div>
                        ...
                        <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example_1">
                            <thead>
                                ...
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase">
                                    <th class="min-w-100px">Centro de Abastecimiento</th>
                                    <th class="min-w-100px">Equipo</th>
                                    <th class="min-w-100px">Odomentro Inicial</th>
                                    <th class="min-w-100px">Odomentro Final</th>
                                    <th class="min-w-100px">Cantidad Galones</th>
                                    <th class="min-w-100px">Kilometraje total</th>
                                    <th class="min-w-100px">Rendimiento General</th>
                            ...
                            <tbody class="fw-bold text-gray-600">
                                ...
                                <tr class="odd">
                                    <td>{{ $booking->supply_center->name ?? 'N/A' }}</td>
                                    <td>{{ $booking->vehicle->name ?? 'N/A' }}</td>
                                    <td>{{ number_format($booking->initial_odometer) }}</td>
                                    <td>{{ number_format($booking->final_odometer) ?? 'N/A'  }}</td>
                                    <td>{{ number_format($booking->number_of_gallons, 2) }}</td>
                                    <td>{{ number_format($booking->total_mileage) }}</td>
                                    <td>{{ number_format($booking->overall_yield, 1) }}</td>
    ...
    <script>
        ...
        var KTDatatablesButtons = function () {
            ...
            // Hook export buttons
            var exportButtons = () => {
                const documentTitle = 'Reporte de Consumo  Desde: ' + from + ' Hasta: ' + to;
